    ...
    return notes.map(note => {
        ...
        if (note.attachment) {
            ...
            if (mediaType === 'image') {
                ...
                attachmentHtml = '<div class="note-attachment"><div class="media-player audio-player" id="' + pid + '">' +
                    '<audio preload="metadata"><source src="' + att.url + '"></audio>' +
                    '<div class="media-controls">' +
                        '<button class="media-btn play-btn" onclick="toggleMediaPlay(\'' + pid + '\')">' +
                            '<svg class="icon-play" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>' +
                            '<svg class="icon-pause" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>' +
...
function getMediaElement(playerId) {
    ...
    return player.querySelector('video') || player.querySelector('audio');
}
...
function toggleMediaPlay(playerId) {
    ...
    if (media.paused) {
        ...
        document.querySelectorAll('.media-player').forEach(function(p) {
            if (p.id !== playerId) {
                var otherMedia = p.querySelector('video') || p.querySelector('audio');
