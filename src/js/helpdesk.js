...
function submitTicket() {
    ...
    if (ticketType === 'support' && !state.selectedCategory) {
        showToast('Please select a category', 'error');
        return;
    }
    if (state.selectedCategory === 'other' && !document.getElementById('customCategory').value.trim()) {
    ...
    window.parent.postMessage({
        ...
        category: ticketType === 'support' ? state.selectedCategory : ticketType,
...
// ==========================================
// HELPDESK WIX BLOCKS WIDGET - COMPLETE
// File: widgets/helpdesk-widget/widget.js
// ==========================================
...
import { 
    ...
    getReferrals,
    addReferral
} from 'backend/helpdesk.web';

import { triggeredEmails } from "@wix/site-crm";

const ADMIN_EMAIL = "<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="5e2a373027702e3f303a3f1e2a373027732e3f303a3f703d3133">[email&#160;protected]</a>";
const ADMIN_MEMBER_ID = "e56a2e68-0414-46ce-a1af-94a142fd5555";
const NEW_TICKET_EMAIL_ID = "V9F4kdl";
const UPDATE_TICKET_EMAIL_ID = "V9FFlsI";

let currentDomain = null;
let currentHtmlComponent = null;
let currentUser = null;
let isAdmin = false;

$w.onReady(async () => {
    console.log("Helpdesk widget ready");
    
    const htmlComponent = $w("#helpdeskHtml");
    ...
    htmlComponent.onMessage(async (event) => {
        ...
        switch (data.action) {
            case 'ready':
                await initializeHelpdesk(htmlComponent);
                break;
            case 'createTicket':
                await handleCreateTicket(htmlComponent, data);
                break;
            case 'addNote':
                await handleAddNote(htmlComponent, data);
                break;
            case 'refreshTickets':
                await loadTickets(htmlComponent);
                break;
            case 'updateStatus':
                await handleUpdateStatus(htmlComponent, data);
                break;
            case 'deleteTicket':
                await handleDeleteTicket(htmlComponent, data);
                break;
            case 'saveProfile':
                await handleSaveProfile(htmlComponent, data);
                break;
            case 'requestUpload':
                $widget.fireEvent('uploadRequested', {});
                handleUploadRequest();
                $w("#boxUploadRoutineInner").scrollTo()
                break;
            case 'pacman':
                $widget.fireEvent('pacman', { text: 'pacman' });
                console.log("Fired pacman event");
                break;
            case 'updateTicketType':
                await handleUpdateTicketType(htmlComponent, data);
                break;
            case 'updateProjectValue':
                await handleUpdateProjectValue(htmlComponent, data);
                break;
            case 'getReferrals':
                await handleGetReferrals(htmlComponent);
                break;
            case 'addReferral':
                await handleAddReferral(htmlComponent, data);
                break;
            case 'refreshContract':
                await loadContractInfo(htmlComponent);
                break;
                // In the switch statement inside handleMessage(), add:
            case 'modalOpened':
                // Optional: fire event to page code
                $widget.fireEvent('modalOpened', { modal: data.modal });
                break;
            case 'notificationReceived':
            $widget.fireEvent('notificationReceived', { 
    ...
    $w('#dropdownFileType').onChange(() => {
        const selectedType = $w('#dropdownFileType').value;
        if (selectedType) {
            $w('#uploadButtonNote').fileType = selectedType;
            $w('#uploadButtonNote').enable();
        } else {
            $w('#uploadButtonNote').disable();
        }
    });

    $w('#uploadButtonNote').onChange(() => {
        handleNoteUpload($w("#helpdeskHtml"));
    });

    $w('#buttonCloseBoxUpload').onClick(() => {
        $w("#boxUploadRoutine").hide();
        $w("#boxUploadRoutine").collapse();
        $w('#dropdownFileType').value = undefined;
        $w('#uploadButtonNote').disable();
        $w("#helpdeskHtml").postMessage({ action: 'uploadCancelled' });
    });
});
...
async function sendNewTicketNotification(ticket) {
    try {
        await triggeredEmails.emailMember(NEW_TICKET_EMAIL_ID, ADMIN_MEMBER_ID, {
            variables: {
                ...
                businessImpact: ticket.businessImpact || 'moderate',
...
async function sendTicketUpdateNotification(ticket, latestNote = '') {
    try {
        await triggeredEmails.emailMember(UPDATE_TICKET_EMAIL_ID, ADMIN_MEMBER_ID, {
            variables: {
                notes: latestNote,
                ticketNumber: ticket.ticketNumber || '',
                userEmail: ticket.userEmail || '',
        ...
        console.log("Ticket update notification email sent");
        return true;
    } catch (error) {
...
async function initializeHelpdesk(htmlComponent) {
    try {
        const userResult = await getCurrentUser();
        
        if (userResult.success && userResult.user) {
            currentUser = userResult.user;
            isAdmin = userResult.user.email && 
                      userResult.user.email.toLowerCase() === ADMIN_EMAIL.toLowerCase();
            
            const profileResult = await getUserProfile();
            
            if (!profileResult.authorized && !isAdmin) {
                htmlComponent.postMessage({
                    action: 'accessDenied',
                    message: profileResult.error || "Access denied. Your email is not registered in the system."
                });
                return;
            }
            
            currentDomain = profileResult.domain || null;
            ...
            if (profileResult.success && profileResult.profile) {
                profile = {
                    ...
                    image: convertWixUriToHttps(profileResult.profile.image)
                };
            }
            ...
            htmlComponent.postMessage({
                ...
                isAdmin: isAdmin,
                profile: profile,
                hasProfile: profileResult.success ? profileResult.hasProfile : false,
                domain: profileResult.domain || null
            });
            ...
            currentHtmlComponent = htmlComponent;
            console.log("Helpdesk initialized");
        } else {
            htmlComponent.postMessage({
                action: 'error',
                message: "Please log in to access the support desk"
            });
        }
    } catch (error) {
        ...
        htmlComponent.postMessage({
            action: 'error',
            message: "Failed to initialize support desk"
        });
    }
}
...
async function loadTickets(htmlComponent) {
    try {
        const result = isAdmin ? await getAllTicketsAdmin() : await getUserTickets();
        ...
        if (result.success) {
            ...
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to load tickets" });
            htmlComponent.postMessage({ action: 'setTickets', tickets: [], users: [], companies: [] });
        }
    } catch (error) {
        console.error("Error loading tickets:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to load tickets" });
    }
}

async function handleCreateTicket(htmlComponent, data) {
    try {
        ...
        if (result.success) {
            sendNewTicketNotification(result.ticket);
            $widget.fireEvent('ticketCreated', { ticket: result.ticket });
            htmlComponent.postMessage({
                action: 'ticketCreated',
                ticket: result.ticket,
                tasksDeducted: result.tasksDeducted || 0
            });
            if (result.tasksDeducted > 0) await loadContractInfo(htmlComponent);
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to create ticket" });
        }
    } catch (error) {
        console.error("Error creating ticket:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to create ticket" });
    }
}

async function handleAddNote(htmlComponent, data) {
    try {
        const result = await addNoteToTicket(data.ticketId, data.content || '', data.attachment || null);
        
        if (result.success) {
            if (!isAdmin && data.ticket) {
                let noteContent = data.content || '';
                if (data.attachment) noteContent += (noteContent ? '\n' : '') + '[Attachment: ' + (data.attachment.filename || 'file') + ']';
                sendTicketUpdateNotification(data.ticket, noteContent);
            }
            $widget.fireEvent('noteAdded', { ticketId: result.ticketId, note: result.note, domain: result.domain });
            htmlComponent.postMessage({ action: 'noteAdded', ticketId: result.ticketId, note: result.note });
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to add note" });
        }
    } catch (error) {
        console.error("Error adding note:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to add note" });
    }
}

async function handleUpdateStatus(htmlComponent, data) {
    try {
        const result = await updateTicketStatus(data.ticketId, data.status);
        
        if (result.success) {
            $widget.fireEvent('statusUpdated', { ticketId: data.ticketId, status: data.status, domain: result.domain });
            htmlComponent.postMessage({ action: 'statusUpdated', ticketId: data.ticketId, status: data.status });
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to update status" });
        }
    } catch (error) {
        console.error("Error updating status:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to update status" });
    }
}

async function handleDeleteTicket(htmlComponent, data) {
    try {
        const result = await deleteTicket(data.ticketId);
        
        if (result.success) {
            $widget.fireEvent('ticketDeleted', { ticketId: data.ticketId, domain: result.domain });
            htmlComponent.postMessage({ action: 'ticketDeleted', ticketId: data.ticketId });
            await loadContractInfo(htmlComponent);
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to delete ticket" });
        }
    } catch (error) {
        console.error("Error deleting ticket:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to delete ticket" });
    }
}

async function handleSaveProfile(htmlComponent, data) {
    try {
        const result = await saveUserProfile(data.name, data.companyName);
        
        if (result.success) {
            htmlComponent.postMessage({ action: 'profileSaved', profile: result.profile });
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to save profile" });
        }
    } catch (error) {
        console.error("Error saving profile:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to save profile" });
    }
}

// ==========================================
// TICKET TYPE & PROJECT VALUE HANDLERS
// ==========================================

async function handleUpdateTicketType(htmlComponent, data) {
    try {
        const result = await updateTicketType(data.ticketId, data.ticketType);
        
        if (result.success) {
            $widget.fireEvent('ticketTypeUpdated', {
                ticketId: data.ticketId,
                ticketType: result.ticketType,
                oldType: result.oldType,
                taskDelta: result.taskDelta,
                domain: result.domain
            });
            htmlComponent.postMessage({
                action: 'ticketTypeUpdated',
                ticketId: data.ticketId,
                ticketType: result.ticketType,
                oldType: result.oldType,
                taskDelta: result.taskDelta
            ...
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to update ticket type" });
        }
    } catch (error) {
        console.error("Error updating ticket type:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to update ticket type" });
    }
}

async function handleUpdateProjectValue(htmlComponent, data) {
    try {
        const result = await updateProjectValue(data.ticketId, data.value);
        
        if (result.success) {
            $widget.fireEvent('projectValueUpdated', {
                ticketId: data.ticketId,
            ...
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to update project value" });
        }
    } catch (error) {
        console.error("Error updating project value:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to update project value" });
    }
}

// ==========================================
// CONTRACT & REFERRAL HANDLERS
// ==========================================

async function loadContractInfo(htmlComponent) {
    try {
        const result = await getContractInfo();
        htmlComponent.postMessage({ action: 'setContractInfo', contract: result.success ? result.contract : null });
    } catch (error) {
        console.error("Error loading contract info:", error);
    }
}

async function handleGetReferrals(htmlComponent) {
    try {
        const result = await getReferrals();
        htmlComponent.postMessage({
            action: 'setReferrals',
            referrals: result.success ? result.referrals : [],
            count: result.success ? result.count : 0
        });
    } catch (error) {
        console.error("Error getting referrals:", error);
        htmlComponent.postMessage({ action: 'setReferrals', referrals: [], count: 0 });
    }
}

async function handleAddReferral(htmlComponent, data) {
    try {
        const result = await addReferral({
            companyReferred: data.companyReferred,
        ...
        if (result.success) {
            htmlComponent.postMessage({ action: 'referralAdded', referral: result.referral, tasksAdded: result.tasksAdded });
            await handleGetReferrals(htmlComponent);
            await loadContractInfo(htmlComponent);
        } else {
            htmlComponent.postMessage({ action: 'error', message: result.error || "Failed to add referral" });
        }
    } catch (error) {
        console.error("Error adding referral:", error);
        htmlComponent.postMessage({ action: 'error', message: "Failed to add referral" });
    }
}
...
function handleRealtimeMessage(payload) {
    ...
    if (!payload || !payload.type || !currentHtmlComponent) return;
    
    switch (payload.type) {
        case 'noteAdded':
            currentHtmlComponent.postMessage({ action: 'realtimeNoteAdded', ticketId: payload.ticketId, note: payload.note });
            break;
        case 'statusUpdated':
            currentHtmlComponent.postMessage({ action: 'realtimeStatusUpdated', ticketId: payload.ticketId, status: payload.status });
            break;
        case 'ticketUpdated':
            currentHtmlComponent.postMessage({ action: 'realtimeTicketUpdated', ticketId: payload.ticketId });
            break;
        case 'ticketCreated':
            currentHtmlComponent.postMessage({ action: 'realtimeTicketCreated', ticket: payload.ticket });
            loadContractInfo(currentHtmlComponent);
            break;
        case 'ticketDeleted':
            currentHtmlComponent.postMessage({ action: 'realtimeTicketDeleted', ticketId: payload.ticketId });
            loadContractInfo(currentHtmlComponent);
            break;
        case 'ticketTypeUpdated':
            currentHtmlComponent.postMessage({ action: 'realtimeTicketTypeUpdated', ticketId: payload.ticketId, ticketType: payload.ticketType });
            loadContractInfo(currentHtmlComponent);
            break;
        case 'projectValueUpdated':
            currentHtmlComponent.postMessage({ action: 'realtimeProjectValueUpdated', ticketId: payload.ticketId, projectValue: payload.projectValue });
            loadContractInfo(currentHtmlComponent);
            break;
        default:
            console.log("Unknown realtime message type:", payload.type);
    }
}
...
function handleUploadRequest(htmlComponent) {
    $w("#uploadButtonNote").reset();
    $w('#dropdownFileType').value = undefined;
    $w('#uploadButtonNote').disable();
    $w("#boxUploadRoutine").expand();
    $w("#boxUploadRoutine").show();
}

async function handleNoteUpload(htmlComponent) {
    if ($w("#uploadButtonNote").value.length === 0) return;
    
    try { $w("#lottieUploadSpinner").show(); $w("#lottieUploadSpinner").expand(); } catch (e) { /* Spinner element may not exist */ }
    
    try {
        const uploadedFiles = await $w("#uploadButtonNote").uploadFiles();
        
        if (uploadedFiles.length > 0) {
            const uploadedFile = uploadedFiles[0];
            const httpsUrl = convertWixUriToHttps(uploadedFile.fileUrl);
            const fileType = detectFileTypeFromName(uploadedFile.originalFileName || "file");
        ...
        htmlComponent.postMessage({ action: 'uploadError', message: uploadError.message || "Upload failed" });
    } finally {
        $w("#uploadButtonNote").reset();
        $w('#dropdownFileType').value = undefined;
        $w('#uploadButtonNote').disable();
        $w("#boxUploadRoutine").hide();
        $w("#boxUploadRoutine").collapse();
        try { $w("#lottieUploadSpinner").hide(); $w("#lottieUploadSpinner").collapse(); } catch (e) { /* Spinner element may not exist */ }
    }
}

function convertWixUriToHttps(wixUri) {
    if (!wixUri) return null;
    if (wixUri.startsWith('https://') || wixUri.startsWith('http://')) return wixUri;
    
    try {
        let fileType = 'image';
        if (wixUri.startsWith('wix:video://')) fileType = 'video';
        else if (wixUri.startsWith('wix:document://')) fileType = 'document';
        else if (wixUri.startsWith('wix:audio://')) fileType = 'audio';
        
        let mediaPath = wixUri;
        if (wixUri.includes('://v1/')) mediaPath = wixUri.split('://v1/')[1];
        mediaPath = mediaPath.split('#')[0];
        const mediaId = mediaPath.split('/')[0];
        
        if (mediaId) {
            switch (fileType) {
                case 'video': return `https://video.wixstatic.com/video/${mediaId}/file`;
                case 'document': return `https://docs.wixstatic.com/ugd/${mediaId}`;
                case 'audio': return `https://static.wixstatic.com/media/${mediaId}`;
                default: return `https://static.wixstatic.com/media/${mediaId}`;
            }
        }
    } catch (e) {
        console.error('Error parsing Wix URI:', e);
    }
    return wixUri;
}
